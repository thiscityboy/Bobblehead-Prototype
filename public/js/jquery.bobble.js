  ...
  $.fn.bobble = function(method) {
    ...
    return this.each(function() {
      ...
      function handleOut(target,pageX,pageY)
      {
        ...
        if (leftAt_x < -120) leftAt_x = -10;
        if (leftAt_y > 20)  leftAt_y = 10;
        if (leftAt_y < -120) leftAt_y = -10;

        //mouse out bounce back animation
        $(target).animate({'left': -leftAt_x,     'top': -leftAt_y,     'rotateZ': deg2rad(-leftAt_r),     'scale': (1-(leftAt_y/500))},    300);
        $(target).animate({'left': leftAt_x*.9,   'top': leftAt_y*.9,   'rotateZ': deg2rad(leftAt_r*.9),   'scale': (1+(leftAt_y/500*.9))}, 300);
        $(target).animate({'left': -leftAt_x*.75, 'top': -leftAt_y*.66, 'rotateZ': deg2rad(-leftAt_r*.66), 'scale': (1-(leftAt_y/500*.66))},300);
        $(target).animate({'left': leftAt_x*.5,   'top': leftAt_y*.5,   'rotateZ': deg2rad(leftAt_r*.5),   'scale': (1+(leftAt_y/500*.5))}, 300);
        $(target).animate({'left': -leftAt_x*.25, 'top': -leftAt_y*.25, 'rotateZ': deg2rad(-leftAt_r*.25), 'scale': (1-(leftAt_y/500*.25))},300);
        $(target).animate({'left': 0,             'top': 0,             'rotateZ': 0,                      'scale': 1},                     500);
      }
